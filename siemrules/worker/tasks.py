    ...
    match job.type:
        case JobType.CORRELATION_SIGMA:
            task = process_correlation.s(
                job.id, correlation.model_dump(by_alias=True), related_indicators
            )
...
def modify_correlation(job_id, indicator, new_rule_data):
    ...
    if modify_type == 'revert':
        version_kwargs["reverted_to"] = indicator['modified']
...
def run_file2txt(file: models.File):
    with tempfile.TemporaryDirectory(prefix="siemrules_") as tmp_dir:
        ...
        if file.profile.generate_pdf and pdf_converter.make_conversion(
        ...
        converter: BaseParser = parser_class(
            str(input_file),
            file.mode,
            file.profile.extract_text_from_image,
        ...
        for name, img in converter.images.items():
            ...
            models.FileImage.objects.create(
                report=file, image=DjangoFile(img_file, name), name=name
            )
...
def upload_to_arango(job: models.Job, bundle: dict, link_collection=True):
    upload_objects(
        ...
        bundle,
        extra_data=dict(
            _stixify_report_id=job.file.report_id, _siemrules_job_id=str(job.id)
        ),
...
def upload_objects(
    job: models.Job, bundle, extra_data: dict, link_collection=False, **kwargs
):
    s2a = Stix2Arango(
        file=None,
    ...
    if link_collection:
        db_view_creator.link_one_collection(
            s2a.arango.db,
            settings.VIEW_NAME,
            f"{settings.ARANGODB_COLLECTION}_edge_collection",
        )
        db_view_creator.link_one_collection(
            s2a.arango.db,
            settings.VIEW_NAME,
            f"{settings.ARANGODB_COLLECTION}_vertex_collection",
        )
...
def process_correlation_ai(job_id, data, related_indicators):
    ...
    correlation = correlations.generate_correlation_with_ai(
        model, data["prompt"], related_indicators
    )
...
def job_failed(request, exc, traceback, job_id=None):
    logging.error(
        "Task {0} with job_id {3} raised exception: {1!r}\n{2!r}".format(
